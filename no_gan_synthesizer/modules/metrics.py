...
def compute_ecdf(data:pd.DataFrame, n_nodes:int = 1000, 
                 adjusted:bool = False, verbose:bool = True)->List:
    """
    Function to compute ecdf on proportion of row counts generated by dynamic pandas queries on a dataset

    Args:
        data (pd.DataFrame): Pandas DataFrame
        n_nodes (int, optional):No of nodes or queries to generate. Defaults to 1000.
        adjusted (bool, optional): Adjustment to be done to the randomly calculated percentiles. Useful when data dimensions are big. Defaults to False.
...
def ks_delta(synthetic_data:pd.DataFrame, ecdf_validation:List)->Tuple:
    """
    Generates filter count based on the validation ECDF List & Calculates the KS Distance between the ECDFs of Synth and Validation.

    Args:
        synthetic_data (pd.DataFrame): Synthetic Pandas DataFrame
        ecdf_validation (List): ECDF list of validation data

    Returns:
        Tuple: Returns a tuple of Max KS Distance and ECDFs of validation and Synthetic Data
    """
